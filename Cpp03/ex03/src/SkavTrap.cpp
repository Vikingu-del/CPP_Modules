...
/*   Updated: 2024/03/08 05:06:25 by eseferi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void SkavTrap::attack( const std::string& target ) {
    if ( this->_energyPoints < 1)
        std::cout << YELLOW << "SkavTrap " << this->_name << " has no energy points left to attack!" << RESET << std::endl;
    else {
        std::cout << YELLOW << "SkavTrap " << this->_name << " attacks " << target << ", causing " << this->_attackDamage << " points of damage!" << RESET << std::endl;
        this->_energyPoints--;
    }
}
